...
TEST(DifferentialPressureSensorTest, Initialize) {
  ...
  EXPECT_CALL(i2c_handle_mock, change_address(kSDP810I2CAddr));
  DifferentialPressureSensor DiffPressSensor = DifferentialPressureSensor(&i2c_handle_mock);
  EXPECT_CALL(i2c_handle_mock, send_bytes(_, kSDP810InitCMDSize)).WillOnce(Invoke(copy_buffer));
  DiffPressSensor.Initialize();
  EXPECT_EQ(testbuffer[0], kContMassFlowAvgMsb);
  EXPECT_EQ(testbuffer[1], kContMassFlowAvgLsb);
...
TEST(DifferentialPressureSensorTest, GetSensorData) {
  ...
  I2CDriver i2c_handle_mock;
  DifferentialPressureSensor DiffPressSensor = DifferentialPressureSensor(&i2c_handle_mock);
  EXPECT_CALL(i2c_handle_mock,read_bytes(_, kSDP810BufferSize)).WillOnce(Invoke(set_example_buffer));
  SensorData data = DiffPressSensor.GetSensorData();
  EXPECT_EQ(data.buffer[0], sensorRaw);
  EXPECT_EQ(data.numOfBytes, kSDP810BytesToReturn);
  Mock::VerifyAndClearExpectations(&i2c_handle_mock);
}
